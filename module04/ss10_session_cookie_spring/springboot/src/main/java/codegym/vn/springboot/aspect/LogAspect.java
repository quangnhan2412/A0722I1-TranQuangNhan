...
public class LogAspect {
    ...
    @Pointcut("execution(* codegym.vn.springboot.service.StudentService.*(..))")
    public void studentService(){};

    @Pointcut("execution(* codegym.vn.springboot.service.StudentService.delete(..))")
    ...
    @After("execution (* codegym.vn.springboot.service.StudentService.fi*(..)) || execution(* codegym.vn.springboot.service.StudentService.delete(..))")
    public void writeLogFindAll(JoinPoint point) throws Exception {
        String methodName = point.getSignature().getName();
        logger.info("After call method: " + methodName);
    }

    @AfterThrowing(value = "execution (* codegym.vn.springboot.service.StudentService.findAll())", throwing = "e")
